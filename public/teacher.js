...
document.getElementById('questionForm').addEventListener('submit', async (e) => {
  ...
  const res = await fetch('https://class-6th-english-2-ch7.onrender.com/add-question', {
    method: 'POST',
...
async function loadQuestions() {
  const res = await fetch('https://class-6th-english-2-ch7.onrender.com/questions');
...
async function deleteQuestion(id) {
  if (confirm('Are you sure you want to delete this question?')) {
    const res = await fetch(`https://class-6th-english-2-ch7.onrender.com/delete-question/${id}`, {
      method: 'DELETE',
    });
